...
class RenderConfusionMatrix():
    ...
    def getYPred(self, datapath, batch_size):
        # Root directory
        data_dir = datapath;
        ##
    ...
    def plotConfusionMatrix(self, cm, classes, normalize=False,
                            title='Confusion matrix', cmap=plt.cm.YlOrBr):
    ...
    def renderConfusionMatrix(self, datapath, batch_size):

        dset_classes, y_true, y_pred = self.getYPred(datapath, batch_size);
        ...
        plt.figure()
        self.plotConfusionMatrix(cnf_matrix, classes=dset_classes[:20], normalize=True,
